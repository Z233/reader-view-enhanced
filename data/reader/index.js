...
const nav = {
  back(forced = false) {
    ...
    if (
      forced === false &&
      (!nav.timeout || now - nav.timeout > 2000)
    ) {
      nav.timeout = now
...
const update = {
  async: () => {
    ...
    styles.internals.textContent = `body {
      ...
      width: ${
        prefs.width ? prefs.width + 'px' : 'calc(100vw - 50px)'
      };
    ...
    document.querySelector('[data-id=no-height] input').checked =
      Boolean(prefs['line-height']) === false
    document.querySelector('[data-id=full-width] input').checked =
      Boolean(prefs.width) === false
    // as a CSS selector
    document.body.dataset.font = prefs.font
    //
    document.querySelector(
      '#font-details [data-id="font-size"]'
    ).textContent = prefs['font-size'] + 'px'
    document.querySelector(
...
/* screenshot */
{
  ...
  span.onclick = () => {
    const next = () =>
      html2canvas(iframe.contentDocument.documentElement).then(
        (canvas) => {
...
/* email */
{
  ...
  span.onclick = () => {
    ...
    a.href =
      'mailto:' +
      config.prefs['mail-to'] +
      '?subject=' +
      encodeURIComponent(article.title.trim()) +
      '&body='
    let body = article.textContent.trim().replace(/\n{4,}/g, '\n\n\n')
    const ending = config.prefs['mail-ending'].replace(
      /\[(\w+)\]/g,
      (a, b) => {
    ...
    const max =
      config.prefs['mail-max'] - a.href.length - ending.length
...
/* speech */
{
  ...
  span.onclick = async () => {
    if (document.body.dataset.speech === 'true') {
      ...
      await add(
        'libs/text-to-speech/vendors/sentence-boundary-detection/sbd.js'
      )
      ...
      tts.on('status', (s) => {
        ...
        ).textContent = s === 'buffering' ? 'Please Wait...' : ''
      })
      tts.on('error', (e) =>
        chrome.runtime.sendMessage({
          cmd: 'notify',
      ...
      const nodes = [
        ...iframe.contentDocument.querySelectorAll(`.page p,
        .page section,
        .page h1,
        .page h2,
        .page h3,
      ...
      if (nodes.length === 0) {
        const article =
          iframe.contentDocument.querySelector('article')
        if (article) {
          nodes.push(article)
        }
      }
      ...
      await tts.ready()
      tts.buttons.play.title = `Play/Pause (${config.prefs.shortcuts[
        'speech-play'
      ]
        .map((s) => s.replace('Key', ''))
        .join(' + ')})`
      tts.buttons.next.title = `Next (${config.prefs.shortcuts[
        'speech-next'
      ]
        .map((s) => s.replace('Key', ''))
        .join(' + ')})`
      tts.buttons.previous.title += `Previous (${config.prefs.shortcuts[
        'speech-previous'
      ]
        .map((s) => s.replace('Key', ''))
        .join(' + ')})`
          ...
          const bounded = tts.sections.filter((e) => {
            for (const c of e.targets ? e.targets : [e.target || e]) {
              ...
              if ((c.target || c).nodeType === Node.ELEMENT_NODE) {
                if (
                  parent.contains(c.target || c) ||
                  (c.target || c)?.contains(parent)
                ) {
                  return true
                }
              }
  ...
  chrome.storage.local.get(
    ...
    (prefs) => {
      document.getElementById('speech').dataset.mode =
        prefs['speech-mode']
      document.querySelector(
        '[data-cmd="minimize-speech"]'
      ).textContent = prefs['speech-mode'] === '' ? '-' : '□'
    }
  )
  shortcuts.push(
    {
...
/* highlight */
{
  ...
  chrome.runtime.onMessage.addListener((request) => {
    if (request.cmd === 'export-highlights' && highlight.used) {
      ...
      request.cmd === 'append-highlights' &&
      request.href === args.get('url').split('#')[0]
    ) {
...
document.addEventListener('click', (e) => {
  ...
  if (cmd.startsWith('font-type-')) {
    ...
    chrome.storage.local.set({
      width: e.target.parentElement.querySelector('input').checked
        ? 600
        : 0,
    })
  } else if (
    cmd === 'line-height-type-1' ||
    cmd === 'line-height-type-2'
  ) {
    chrome.storage.local.set({
    ...
    chrome.storage.local.set({
      'line-height': e.target.parentElement.querySelector('input')
        .checked
        ? 28.8
        : 0,
    })
    ...
    if (active === false) {
      ...
      ;[...document.querySelectorAll('.edit-toolbar')].forEach(
        (e) => {
          const a = e.contentDocument.querySelector(
            '[data-command="close"]'
          )
  ...
  chrome.runtime.sendMessage(
    ...
    async (obj) => {
      ...
      document.title = document.oTitle = config.prefs.title
        .replace(
          '[ORIGINAL]',
          article.title.replace(' :: Reader View', '')
        )
      ...
      iframe.contentDocument.write(
        (await template())
          ...
          .replaceAll(
            '%light-color%',
            gcs.getPropertyValue('--color-mode-light-color')
          )
          .replaceAll(
            '%light-bg%',
            gcs.getPropertyValue('--color-mode-light-bg')
          )
          .replaceAll(
            '%dark-color%',
            gcs.getPropertyValue('--color-mode-dark-color')
          )
          .replaceAll(
            '%dark-bg%',
            gcs.getPropertyValue('--color-mode-dark-bg')
          )
          .replaceAll(
            '%sepia-color%',
            gcs.getPropertyValue('--color-mode-sepia-color')
          )
          .replaceAll(
            '%sepia-bg%',
            gcs.getPropertyValue('--color-mode-sepia-bg')
          )
          .replaceAll(
            '%solarized-light-color%',
            gcs.getPropertyValue('--color-mode-solarized-light-color')
          )
          .replaceAll(
            '%solarized-light-bg%',
            gcs.getPropertyValue('--color-mode-solarized-light-bg')
          )
          .replaceAll(
            '%nord-light-color%',
            gcs.getPropertyValue('--color-mode-nord-light-color')
          )
          .replaceAll(
            '%nord-light-bg%',
            gcs.getPropertyValue('--color-mode-nord-light-bg')
          )
          .replaceAll(
            '%groove-dark-color%',
            gcs.getPropertyValue('--color-mode-groove-dark-color')
          )
          .replaceAll(
            '%groove-dark-bg%',
            gcs.getPropertyValue('--color-mode-groove-dark-bg')
          )
          .replaceAll(
            '%solarized-dark-color%',
            gcs.getPropertyValue('--color-mode-solarized-dark-color')
          )
          .replaceAll(
            '%solarized-dark-bg%',
            gcs.getPropertyValue('--color-mode-solarized-dark-bg')
          )
          .replaceAll(
            '%nord-dark-color%',
            gcs.getPropertyValue('--color-mode-nord-dark-color')
          )
          .replaceAll(
            '%nord-dark-bg%',
            gcs.getPropertyValue('--color-mode-nord-dark-bg')
          )
      ...
      // fix relative links;
      const es = [
        ...iframe.contentDocument.querySelectorAll('[src^="//"]'),
      ]
      for (const e of es) {
        e.src =
          article.url.split(':')[0] + ':' + e.getAttribute('src')
      }
      ...
      // navigation
      {
        ...
        previous.onclick = next.onclick = (e) => {
          ...
          guide.timeout = setTimeout(
            () => guide.classList.add('hidden'),
            config.prefs['guide-timeout']
          )
        }
        const scroll = () => {
      ...
      iframe.contentDocument.addEventListener(
        ...
        () => {
          ...
          try {
            if (
              iframe.contentDocument.activeElement.classList.contains(
                'note'
              )
            ) {
...
config.load(() => {
  ...
  if (config.prefs['design-mode-button']) {
    document
      .getElementById('design-mode-button')
      .classList.remove('hidden')
  }
  update.images()
  update.async()
